...
public class ServerMapManager: NetworkBehaviour {
	...
	private delegate void OnMapUpdateFunc();
	private OnMapUpdateFunc _onMapUpdate = null;


	private void OnDishOutCallback(object sender, ServerConvertEventArgs args) => this.OnDishOut?.Invoke(sender, new DishOutEventArgs((ICombinableSO)args.target.Info));
	...
	public void ResetMapArray() {
		this._targetDishesSO = null;
		this.TargetDishesSOStrKeys.Clear();
		this.TargetDishesAmount.Value = 0;
		this._mapCellState = null;
	}
		...
		for (int x = 0; x < _mapXSize; x++)
			for (int y = 0; y < _mapYSize; y++)
				switch (this._mapCellState[x, y]) {
					...
					case CellState.PlayerSpawn:
						break;
					case CellState.Unwalkable:
					case CellState.Table:
						GameObject tableOnly = Instantiate(this._mapComponentPrefabs.TablePrefab, this.GetWorldCoor(x, y), Quaternion.identity, this._mapParent);
						...
						this._onMapUpdate += tableServer.OnMapUpdate;
						break;
					case CellState.Utility:
						GameObject stationaryUtility = Instantiate(this._mapComponentPrefabs.UtilityPrefab, this.GetWorldCoor(x, y), Quaternion.identity, this._mapParent);
						...
						this._onMapUpdate += stationaryUtilityServer.OnMapUpdate;
						break;
					case CellState.IngredientSpawn:
						GameObject spawnable = Instantiate(this._mapComponentPrefabs.SpawnablePrefab, this.GetWorldCoor(x, y), Quaternion.identity, this._mapParent);
						...
						this._onMapUpdate += spawnableServer.OnMapUpdate;
						break;
					//              case CellState.Tool:
					//                  GameObject table = Instantiate(this._mapComponentPrefabs.TablePrefab, this.GetWorldCoor(x, y), Quaternion.identity, this.transform);
					//                  tableServer = table.GetComponent<IServerHolder>();
					//                  tableServer.InfoInit(this._basicComponentSO.TableSO);
					//                  table.GetComponent<NetworkObject>().Spawn();
					//this.OnMapDespawn += tableServer.OnMapDespawn;

					//                  IServerGrabbable toolServer = Instantiate(this._mapComponentPrefabs.ToolPrefab, this.GetWorldCoor(x, y), Quaternion.identity, this.transform).GetComponent<IServerGrabbable>();
					...
					case CellState.DishExit:
						GameObject dishExit = Instantiate(this._mapComponentPrefabs.DishExitPrefab, this.GetWorldCoor(x, y), Quaternion.identity, this._mapParent);
						...
						this._onMapUpdate += dishExitServer.OnMapUpdate;
						dishExitServer.OnConvert +=
							(sender, args) => this._orderManager.OnDishOut(sender, new DishOutEventArgs(args.target.Info)); // for scoring
						break;
					default:
	...
	private void IdentifyRquiredInteractableList(ICombinableSO[] targetDishesSO, out List<ICombinableSO> requiredCombinableSOList, out List<IConverterSO> requiredConverterSOList) {
		...
		foreach (ICombinableSO curCombinableSO in requiredCombinableSOList)
			combinableListMsg += " " + curCombinableSO.ToString() + ",";
		foreach (IConverterSO curUsableHolderSO in requiredConverterSOList)
			stationaryUtilityListMsg += " " + curUsableHolderSO.ToString() + ",";
			...
			for (int y = 0; y < this._mapYSize; y++) {
				switch (this._mapCellState[x, y]) {
					case CellState.Walkable:
						...
						Gizmos.DrawWireCube(this.GetWorldCoor(x, y) + (new Vector3(0, 10, 0)), new Vector3(cellXSize, 1, cellYSize));
						break;
					case CellState.Unwalkable:
	...
	public void OnMapUpdate() {
		this._onMapUpdate?.Invoke();
	}

	public Vector3 GetWorldCoor(int x, int y) {
		return new Vector3(this._origin.x + this.cellXSize * (x - 1), this._mapZHeight, this._origin.y + this.cellYSize * (y - 1));
	}
