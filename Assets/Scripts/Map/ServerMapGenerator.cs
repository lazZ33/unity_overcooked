...
public class ServerMapGenerator {
	...
	public CellState[,] GenerateMapArray(out Vector2Int[] playerSpawnPoints, int playerAmount, int XSize, int YSize, int utilityAmount, int spawnAmount, int toolAmount, int interactableGroupAmount) {
		...
		for (int x = 0; x < this._mapX; x++) {
			for (int y = 0; y < this._mapY; y++) {
				...
				if (this.IsBorderCell(curCell))
					curCell.cellState = CellState.Unwalkable;
				else
					curCell.cellState = defaultCellState;
			}
		}
		...
		if (isFilledMap) {
			...
			foreach (Cell startCell in pivotsCells) {
				foreach (Cell endCell in pivotsCells) {
					...
					foreach (Cell cell in path) {
						if (cell.cellState == CellState.Unwalkable && !this.IsBorderCell(cell))
							cell.cellState = CellState.Walkable;

						foreach (Cell neighbour in this.Get8Neighbour(cell)) {
							if (neighbour.cellState == CellState.Unwalkable && !this.IsBorderCell(neighbour))
								neighbour.cellState = CellState.Walkable;
						}
					}
			...
			foreach (Cell startCell in pivotsCells) {
				foreach (Cell endCell in pivotsCells) {
					if (UnityEngine.Random.Range(0, 10) > obstaclesChance)
						continue;
		...
		cell => {
			if (cell.cellState != CellState.Unwalkable)
				return false;
			foreach (Cell neighbour in this.Get4Neighbour(cell)) {
				if (neighbour.cellState == CellState.Walkable)
					return true;
			}
			return false;
		});
		...
		for (int i = 0; i < interestedCells.Count; i++) {
			if (i >= utilityAmount + spawnAmount)
				interestedCells[i].cellState = CellState.DishExit;
			else if (i >= utilityAmount)
				interestedCells[i].cellState = CellState.IngredientSpawn;
			else
				interestedCells[i].cellState = CellState.Utility;
		}
		...
		List<Cell> validPlayerSpawnPoints = this.GetRandomCells(playerAmount, (cell) => {
			if (cell.cellState != CellState.Walkable)
				return false;
			foreach (Cell neighbour in this.Get8Neighbour(cell)) {
				if (neighbour.cellState != CellState.Walkable)
					return false;
	...
	private bool IsMapFullyAccessible() {
		...
		for (int x = 0; x < this._mapX; x++) {
			for (int y = 0; y < this._mapY; y++) {
				Cell curCell = this.GetCell(x, y);
				if (curCell.cellState == CellState.Unwalkable)
					continue;
				expectedAccessibleCellCount++;
				if (startPoint == null)
					startPoint = curCell; // pick arbitary walkable cell to start flood fill
			}
		}
	...
	private List<Cell> Get4Neighbour(Cell target) {
		...
		for (int dx = -1; dx < 2; dx += 2) {
			...
			if (0 > neighbourX | neighbourX > this._mapX - 1)
				continue;
			if (0 > target.y | target.y > this._mapY - 1)
				continue;
			Cell neighbourCell = this.GetCell(neighbourX, target.y);
			if (neighbourCell == null)
				Debug.LogWarning("Get4Neighbour null occured");
			neighbours.Add(neighbourCell);
		}
		for (int dy = -1; dy < 2; dy += 2) {
			int neighbourY = target.y + dy;
			if (0 > target.x | target.x > this._mapX - 1)
				continue;
			if (0 > neighbourY | neighbourY > this._mapY - 1)
				continue;
	...
	private List<Cell> GetRandomCells(int amount, int requestedMinTotalDist, int requestedMinDist, Predicate<Cell> requirement) {
		...
		while (totalDist < requestedMinTotalDist | notFufillingMinDist | !cells.TrueForAll(requirement)) {
			...
			if (curCell == null)
				Debug.Log("GetRandomCell generated null cell");
			cells[idx] = curCell;
	...
	private List<Cell> GetPath(Cell startCell, Cell endCell, bool requireWalkable) {
		...
		if (startCell.PosEquals(endCell))
		...
		while (true) {
			// check if path exist
			if (requireWalkable && walkableCells.Count == 0)
				return new List<Cell>(0);
			...
			if (curCell.PosEquals(endCell))
				break;

			// add suitable neighbour into walkable list
			foreach (Cell neighbour in this.Get4Neighbour(curCell)) {
				if (walkedCells.Contains(neighbour) | (this.IsBorderCell(neighbour) && !neighbour.PosEquals(endCell)))
					continue;
				if (requireWalkable && neighbour.cellState != CellState.Walkable && !neighbour.PosEquals(endCell))
					continue;
	...
	private void GenerateObstacle(Cell obstacleSeedCell, int obstacleMaxSize, int fillExtent) {
		...
		for (int i = 0; i < obstacleMaxSize; i++) {
			...
			foreach (Cell obstacleEdgeCell in curObstacleEdgeCells) {
				...
				if (obstacleEdgeCell.cellState != CellState.Walkable)
			...
			if (!this.IsMapFullyAccessible()) {
				foreach (Cell obstacleEdgeCell in curObstacleEdgeCells) {
					if (obstacleEdgeCell.cellState != CellState.Unwalkable)
						continue; // just in case
	...
	private class AStarCellCostComparer: IComparer<Cell> {
		int IComparer<Cell>.Compare(Cell cell1, Cell cell2) {
			if (cell1.GCost == cell2.GCost && cell1.HCost == cell2.HCost)
				return 0;
			else if (cell1.FCost == cell2.FCost && cell1.HCost < cell2.HCost)
				return -1;
			else if (cell1.FCost < cell2.FCost)
				return -1;
			return 1;
		}
	}
